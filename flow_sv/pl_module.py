...
class LitFlow(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict(
            {f'loss{i}': losses[i] for i in range(len(losses))},
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0, mode='valid'):
        ...
        epe = torch.sqrt(((flow - flow_gt)**2).sum(1)).mean()
        ...
        self.log_dict(
            {
                f'{mode}/loss': loss,
                f'{mode}/scale_coef': flow_scale_coef,
                f'{mode}/EPE': epe,
            },
            on_step=False, on_epoch=True, prog_bar=False, logger=True
        )
